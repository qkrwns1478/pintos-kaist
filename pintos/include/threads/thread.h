...
#include <debug.h>               // ASSERT, PANIC 등의 디버깅 매크로 제공
#include <list.h>                // 리스트 자료구조 제공 (thread 리스트 등)
#include <stdint.h>              // 정수형 자료형 정의 (int32_t 등)
#include "threads/interrupt.h"  // 인터럽트 프레임 정의
#include "threads/synch.h"      // 세마포어, 락, 조건변수 등의 동기화 도구 정의
#ifdef VM
#include "vm/vm.h"              // 가상 메모리 서포트 (선택적)
#endif

/* 스레드 생명 주기 상태 정의 */
enum thread_status {
	THREAD_RUNNING,     /* 현재 CPU에서 실행 중 */
	THREAD_READY,       /* 실행 대기 중 */
	THREAD_BLOCKED,     /* 이벤트를 기다리며 블록된 상태 */
	THREAD_DYING        /* 종료 대기 중 (소멸 예정) */
};
